...
const AppointmentScheduleTab: React.FC = () => {
  ...
  const fetchBookings = async () => {
    try {
      ...
      setError((err as Error)?.message || "Không thể tải danh sách lịch hẹn");
    } finally {
      setLoading(false);
    }
  };
  ...
  const groupedByBooking = upcomingAppointments.reduce((acc, apt) => {
    ...
    return acc;
  }, {} as Record<number, typeof upcomingAppointments>);
  ...
  const renderVaccineTimeline = (appointments: typeof upcomingAppointments) => {
  ...
  return (
    <div>
      <div className="mb-4">
        ...
        <Text type="secondary">
          Tổng cộng {upcomingAppointments.length} lịch hẹn từ {selfBookings.length + familyBookings.length} vaccine
        </Text>
      </div>
      ...
      {selfBookings.length > 0 && (
        <div className="mb-6">
          <div className="mb-3 flex items-center gap-2">
            <Title level={5} className="!mb-0">Lịch tiêm của bạn</Title>
      ...
      {familyBookings.length > 0 && (
        <div className="mb-6">
          <div className="mb-3 flex items-center gap-2">
            <Title level={5} className="!mb-0">Lịch tiêm cho thành viên gia đình</Title>
