...
import { Avatar, Badge, Button, Card, Skeleton, Statistic, Typography, Tooltip } from "antd";
...
const CardInfoUser = ({ setOpen, editMode, setEditMode, onOpenSettings }: InfoUserProps) => {
  const user = useAuthStore(state => state.user);
  const isAuthenticated = useAuthStore(state => state.isAuthenticated);

  const memberSince = useMemo(() => {
    if (!user?.birthday) return "--";
    // Tạm dùng birthday để hiển thị, khi backend có createdAt thì thay thế
    try { return dayjs(user.birthday).format("MMM YYYY"); } catch { return "--"; }
  }, [user]);
