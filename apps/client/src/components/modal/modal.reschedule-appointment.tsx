...
import { AppointmentDetail } from "@/types/backend";
...
const RescheduleAppointmentModal: React.FC<RescheduleAppointmentModalProps> = ({
  ...
  const handleSubmit = async () => {
    try {
      ...
      message.error((error as Error)?.message || "Không thể thay đổi lịch hẹn");
    } finally {
      setLoading(false);
    }
  };
    ...
    >
      ...
      <div className="mt-4 p-3 bg-blue-50 rounded-lg">
        <div className="text-xs text-gray-600 space-y-1">
          ...
          <div>
            • Sau khi thay đổi, vui lòng kiểm tra lại thông tin xác nhận
          </div>
        </div>
      </div>
    </Modal>
